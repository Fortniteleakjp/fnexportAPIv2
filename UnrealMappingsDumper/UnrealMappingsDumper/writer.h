...
public:
    ...
    ~FileWriter()
    {
        // fnexportAPI local patch: the host passes an absolute path, so a directory that cannot be
        // written must not take the whole game down through fclose(nullptr).
        if (m_File)
            std::fclose(m_File);
    }
