...
FDecodeResult FRadAudioInfo::Decode(uint8_t* CompressedData, const int32_t CompressedDataSize, uint8_t* OutPCMData, const int32_t OutputPCMDataSize, uint32_t NumChannels, RadAudioDecoderHeader* Decoder)
{
    ...
    while (RemnOutputFrames)
    {
        ...
        // Guarded on the local buffer actually holding data: the reservoir lives for this call
        // only, while the counters live in the (possibly reused) decoder, so a stale
        // "valid frames" count from a previous call must not read a freed/empty buffer.
        if (!OutputReservoir.empty() && Decoder->OutputReservoirReadFrames < Decoder->OutputReservoirValidFrames)
        {
            uint32_t AvailableFrames = Decoder->OutputReservoirValidFrames - Decoder->OutputReservoirReadFrames;
        ...
        // Resync on a corrupted/misaligned block. Advance marker-by-marker to the next 0x55
        // and re-examine; keep trying subsequent markers (the buffer strictly shrinks each step,
        // so this is bounded) and stop if none make the block valid.
        if (BlockResult != RadAExamineBlockResult::Valid)
        {
            bool resynced = false;
            while (RemnCompressedDataSize > 1)
            {
                uint32_t marker = 0;
                bool found = false;
                for (uint32_t i = 1; i < RemnCompressedDataSize; i++)
                {
                ...
                size_t newSize = RemnCompressedDataSize - marker;
                std::memmove(CompressedData, CompressedData + marker, newSize);
                RemnCompressedDataSize = (uint32_t)newSize;
        ...
        if (RemnOutputFrames < RadADecodeBlock_MaxOutputFrames)
        {
            if (OutputReservoir.empty())
                OutputReservoir.resize(RadADecodeBlock_MaxOutputFrames * SampleStride);
    ...
    Result.DataLength = OutputPCMDataSize - (RemnOutputFrames * SampleStride);
