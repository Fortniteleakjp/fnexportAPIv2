...
public sealed class LocalizedOperationFilter : IOperationFilter
{
    ...
    private static readonly Dictionary<string, (Localized Ja, Localized En)> Operations = new()
    {
        ...
        ["BackupController.Download"] = L("FModel用.fbkpを取得", "現在マウント中のビルドのファイル一覧をFModelのバックアップ形式(.fbkp)で返します。FModelの差分読み込みに使えます。", "Download an FModel .fbkp backup", "Returns the mounted build's file list as an FModel backup (.fbkp), usable for FModel's differential loading.", ".fbkpバイナリ", "The .fbkp binary."),
    ...
    private static readonly Dictionary<string, Dictionary<string, (string Ja, string En)>> Parameters = new()
    {
        ...
        ["BackupController.GetInfo"] = P(("name", "ファイル名のベース名。既定はFortniteGame。", "Base name for the file name; default FortniteGame."), ("includePayloads", ".uexp/.ubulk等のペイロードを含めるか。FModelは除外します。", "Include .uexp/.ubulk payload files; FModel excludes them.")),
        ["BackupController.Download"] = P(("name", "ファイル名のベース名。既定はFortniteGame。", "Base name for the file name; default FortniteGame."), ("includePayloads", ".uexp/.ubulk等のペイロードを含めるか。FModelは除外します。", "Include .uexp/.ubulk payload files; FModel excludes them."), ("compress", "FModelと同じLZ4フレームで圧縮するか。既定はtrue。", "Wrap the body in the LZ4 frame FModel writes; default true."), ("cancellationToken", "リクエストのキャンセル状態。", "Request cancellation state.")),
